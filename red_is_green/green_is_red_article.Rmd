...
The candidates also fill in the above mentioned candidate test. This test has questions regarding different political issues and can be answered on a 5 point scale. The results of the test can then be used by the voters to discover candidates they agree or disagree with. Both candidates and voters fill out the same survey and the results are then compared with each other to find the best match in political convictions.

```{r, echo=FALSE}
fig_match <- figure_nums(name = "match",  caption = "Example of match between voters and candidates")
```

In the figure below is an example of a match between a voter and candidates in a Copenhagen Municipality. The math behind the matching done by Altinget is not made publicly available.
...
Many scholars have argued that the socialist-capitalist dimension cannot stand alone [@inglehart1971silent; @kitschelt1994transformation; @kitschelt1997radical; @kriesi2006globalization; @kriesi2008west] and that "party competition in most polities is structured by a second orthogonal dimension, tapping cultural, communal, and environmental issues." [@evans2013political]. Therefore I am building a second green index to see if this is indeed a seperate orthogonal dimension in Danish politics. And the green axis is build with the candidates answers to the following questions:
